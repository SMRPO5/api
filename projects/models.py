...
class Board(BaseModel):
	...
	def has_silver_bullet(self):
		...
		if Column.objects.filter(board=self, high_priority_column=True).exists():
			last_requested_column = Column.objects.filter(board=self, high_priority_column=True)[0]
